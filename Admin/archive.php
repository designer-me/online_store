...
if(isset($_GET['enable'])){
    $delete_id = $_GET['enable'];
    $sq = $mysqli->query("UPDATE `skyevo_product` SET `status`= 1 where id = '$delete_id'");
    echo '<script>alert("Product has been enabled successfully");window.location.assign("archive")</script>';
}
//session_start();
//if(!$_SESSION['email']){
  //  header('Location:index.php'); 
...
//    $sq = $conn->query("DELETE FROM `products` WHERE id = '$_GET[Delete]'");
                ...
                <div class="col-lg-10">
                    ...
                    <div class="row" style="border-top: 1px solid black;">
                        <ol class="breadcrumb">
                            ...
                            <li><a href="archive">Disable Products</a></li>
                        </ol> 
                    </div>
                    <!--BREADCRUMB-->
                    <div class="col-lg-12">
                        <h2 class="text-center">Disable Products</h2><hr/>
                        <div class="col-lg-12">
                            <table class="table table-bordered table-striped" id="example">
                                ...
                                <tbody>
                                    <?php
                                        $sql = $mysqli->query("select * from skyevo_product where status = '0' order by id desc");
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <!--<a href="editproducts.php?Edit=<?= $pub['id']; ?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-pencil"></span></a>-->
                                            <a href="archive?enable=<?= $product['id']; ?>" title="Enable Product" data-toggle="tooltip" class="btn btn-xs btn-default" onclick="return confirm('Are you sure you want to enable this item?');"><span class="glyphicon glyphicon-check"></span></a>
                                        </td>
